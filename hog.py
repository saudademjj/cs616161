...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    currscore = player_score % 10
    nextscroe = (opponent_score // 10)%10

    newscore = 3 * abs(nextscroe - currscore)
    if newscore == 0:
        return 1
    else:
        return newscore
...
def sus_points(score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    factor_numbers = num_factors(score)
    if factor_numbers == 3 or factor_numbers== 4:
...
def play(strategy0, strategy1, update, score0=0, score1=0, dice=six_sided, goal=GOAL):
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            thenumber_0 = strategy0(score0, score1)
            score0 = update(thenumber_0,score0,score1,dice)
            who = 1
        else:
            thenumber_1 = strategy1(score1, score0)
            score1 = update(thenumber_1, score1, score0, dice)
            who = 0
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    base_num = 0
    i = 0
    total = make_averaged(roll_dice, times_called)
    for i in range(1,11):
        score = total(i,dice)
        if score > max_num:
            max_num = score
...
def boar_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    total = boar_brawl(score,opponent_score)
    if total >= threshold:
        return 0
    else:
        return num_rolls
